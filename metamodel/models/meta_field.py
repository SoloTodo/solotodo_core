...
class MetaField(models.Model):
    ...
    def get_form_field(self):
        ...
        class MetaSelectMultiple(forms.SelectMultiple):
            def __init__(self, meta_field, attrs=None, choices=()):
                ...
                self.meta_field = meta_field
                super(MetaSelectMultiple, self).__init__(
    ...
    def save(self, *args, **kwargs):
        ...
        def create_necessary_instance_fields():
            if isinstance(default, InstanceModel) and \
        ...
        if self.hidden and not self.nullable and not self.multiple:
            raise IntegrityError('Hidden fields must be either nullable '
                                 'or multiple')
        ...
        if self.pk:
            ...
            if old_field.model != self.model:
                raise IntegrityError('Can\'t change model after creating '
                                     'the field')
            ...
            if self.requires_default_value_for_saving() and default is None:
                raise IntegrityError('New non-nullable non-multiple fields '
                                     'must provide a default for '
                                     'auto-created rows')

            if not self.requires_default_value_for_saving() \
