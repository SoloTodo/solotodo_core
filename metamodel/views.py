...
from metamodel.serializers import (
    MetaModelWithoutFieldsSerializer,
    MetaModelSerializer,
    InstanceModelSerializer,
    MetaFieldSerializer,
    MetaModelAddFieldSerializer,
    InstanceFieldSerializer,
...
class ModelAddView(FormView):
    ...
    def get_success_url(self):
        return reverse("metamodel_model_meta", kwargs={"pk": self.meta_model.pk})
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("metamodel_model_meta", kwargs={"pk": self.get_object().pk})
            )
        else:
...
class ModelAddInstance(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        form_action = reverse(
            "metamodel_model_add_instance", kwargs={"pk": self.object.pk}
        )
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            instance_model.update_fields(
                form.cleaned_data, request.POST, creator_id=request.user.id
            )
            ...
            if self.is_popup():
                ...
                messages.success(
                    request,
                    "<a href={0}>{1}</a> creada correctamente".format(
                        reverse(
                            "metamodel_instance_detail",
                            kwargs={"pk": instance_model.pk},
                        ),
                        str(instance_model),
                    ),
                ...
                return HttpResponseRedirect(
                    reverse("metamodel_model_detail", kwargs={"pk": meta_model.pk})
                )
        else:
            self.object = meta_model
            context = self.get_context_data(**kwargs)
        ...
        if request.headers.get("X-Requested-With", None) == "XMLHttpRequest":
            ...
            if model_type:
                ...
                if meta_model.is_primitive():
                    ...
                    default_choices = [
                        (e.pk, str(e)) for e in meta_model.instancemodel_set.all()
                    ]

            return HttpResponse(json.dumps([requires_default, default_choices]))

        else:
            return super(MetaModelAddFieldView, self).get(request, *args, **kwargs)
            ...
            if meta_field.requires_default_value_for_saving():
                if meta_field.model.name == "FileField":
                    ...
                    if hasattr(settings, "METAMODEL"):
                        path = os.path.join(
                            settings.METAMODEL.get("MEDIA_PATH", ""), cleaned_default
                        )
                    else:
...
class MetaFieldDetailView(DetailView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(
                reverse("metamodel_model_meta", kwargs={"pk": meta_field.parent.pk})
            )
        else:
    ...
    def get_form(self, data=None):
        ...
        if not meta_field.multiple:
            if meta_field.model.is_primitive():
                form = MetaFieldMakeNonNullablePrimitiveForm(meta_field, data=data)
            else:
                form = MetaFieldMakeNonNullableMetaFieldForm(meta_field, data=data)

        return form

    def get_context_data(self, **kwargs):
        context = super(MetaFieldMakeNonNullableView, self).get_context_data(**kwargs)
...
class InstanceModelDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(InstanceModelDetailView, self).get_context_data(**kwargs)
        ...
        for PluginKlass in Plugin.registry.values():
            plugin_value = PluginKlass.on_instance_model_detail_view(instance_model)
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "<a href={0}>{1}</a> actualizado correctamente".format(
                    reverse(
                        "metamodel_instance_detail", kwargs={"pk": instance_model.pk}
                    ),
                    str(instance_model),
                ),
            )
            if "save" in request.POST:
                return HttpResponseRedirect(
                    reverse(
                        "metamodel_model_detail", kwargs={"pk": instance_model.model.pk}
                    )
                )

            return HttpResponseRedirect(
                reverse("metamodel_instance_detail", kwargs={"pk": instance_model.pk})
            )
...
class InstanceModelDeleteView(DetailView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        messages.success(request, "Instancia borrada correctamente")

        return HttpResponseRedirect(
            reverse("metamodel_model_detail", kwargs={"pk": model.id})
        )
...
class InstanceModelPopupRedirect(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(InstanceModelPopupRedirect, self).get_context_data(**kwargs)
...
class MetaModelViewSet(viewsets.ModelViewSet):
    ...
    def add_instance(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            instance_model.save(initial=True)
            instance_model.update_fields(
                form.cleaned_data, request.data, creator_id=request.user.id
            )
    ...
    def add_field(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if "default" in request.data:
                ...
                meta_field.save()
            serializer = MetaFieldSerializer(meta_field, context={"request": request})
    ...
    def get_dependencies(self, request, pk, *args, **kwargs):
        ...
        serializer = MetaFieldSerializer(
            dependencies, context={"request": request}, many=True
        )
        return Response(serializer.data)
    ...
    def destroy(self, request, *args, **kwargs):
        meta_model = self.get_object()
        dependencies = (
            MetaField.objects.select_related("model")
            .filter(model__id=meta_model.id)
            .count()
        )
        if not meta_model.is_primitive() and dependencies == 0:
            ...
            return Response(
                {"errors": "this meta model has dependencies"},
...
class InstanceModelViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = InstanceModel.objects.select_related("model").prefetch_related(
        "model__fields__model",
        "model__fields__parent",
        "fields__field__model",
        "fields__field__parent",
        "fields__value",
    )
    ...
    def edit(self, request, pk, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return Response(
                InstanceModelSerializer(
                    instance_model, context={"request": request}
                ).data
            )
        else:
            return Response(form.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_dependencies(self, request, pk, *args, **kwargs):
        instance_model = self.get_object()
        dependencies = InstanceField.objects.select_related(
            "parent", "field", "value"
        ).filter(value__id=instance_model.id)
        serializer = InstanceFieldSerializer(
            dependencies, context={"request": request}, many=True
        )
        return Response(serializer.data)
    ...
    def destroy(self, request, *args, **kwargs):
        instance_model = self.get_object()
        dependencies = InstanceField.objects.filter(value__id=instance_model.id).count()
        if not instance_model.is_model_primitive() and dependencies == 0:
            instance_model.delete()
            return Response({"status": "ok"})
        else:
            return Response(
                {"errors": "this instance model has dependencies"},
                status=status.HTTP_400_BAD_REQUEST,
            )


class InstanceFieldViewSet(viewsets.ModelViewSet):
    queryset = InstanceField.objects.select_related(
        "field", "field__model", "field__parent", "value"
    )
    pagination_class = InstancePagination
