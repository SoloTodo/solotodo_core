...
class ProductCreatorListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return [(u.id, u.email) for u in SoloTodoUser.objects.filter(is_staff=True)]
...
class ProductModelAdmin(admin.ModelAdmin):
    list_display = ["__str__", "category", "creator", "creation_date", "last_updated"]
...
class CategorySpecsFilterModelAdmin(admin.ModelAdmin):
    def meta_model_link(self, obj):
        return mark_safe(
            '<a href="/metamodel/models/{}">{}</a>'.format(
...
class VisitModelAdmin(admin.ModelAdmin):
    ...
    def product_link(self, obj):
        ...
        backend_link = "{}products/{}".format(settings.BACKEND_HOST, obj.product_id)

        return format_html(
            "<a href='{url}'>{product}</a>", url=backend_link, product=str(obj.product)
        )
    ...
    def website_link(self, obj):
        ...
        return format_html(
            "<a href='{url}'>{product}</a>", url=website_url, product=str(obj.website)
        )
    ...
    def get_queryset(self, request):
        return Visit.objects.select_related(
            "product__instance_model__model__category", "website", "user"
        )


@admin.register(Rating)
class RatingModelAdmin(admin.ModelAdmin):
    list_display = ("__str__", "product", "store", "creation_date", "status")
    list_filter = ("store",)
...
class StoreSectionAdmin(admin.ModelAdmin):
    def get_queryset(self, request):
        qs = super(StoreSectionAdmin, self).get_queryset(request)
