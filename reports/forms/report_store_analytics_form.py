...
class ReportStoreAnalyticsForm(forms.Form):
    ...
    def generate_report(self):
        ...
        '''
        if selected_store:
            retailer_case_count = f'CASE WHEN retailer_id = {selected_store.id} THEN 1 ELSE NULL END'
            retailer_case_price = f'CASE WHEN retailer_id = {selected_store.id} THEN price ELSE 0 END'
            select_cause += f'''
        ...
        products_dict = {
            product.id: {'name': product.name,
                         ...
                         'specs': product.specs} for product in products}
        ...
        if selected_category:
            spec_columns = CategoryColumn.objects.filter(
                field__category=selected_category,
                purpose__name='reports'
