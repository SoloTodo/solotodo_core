...
from reports.forms.report_websites_traffic_form import ReportWebsitesTrafficForm
...
class ReportViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def store_analysis(self, request):
        ...
        send_store_analysis_report_task.delay(user.id, request.META["QUERY_STRING"])
    ...
    def weekly_prices(self, request):
        ...
        send_weekly_prices_task.delay(user.id, request.META["QUERY_STRING"])
    ...
    def product_list(self, request):
        report = Report.objects.get(slug="product_list")
        user = request.user

        if not user.has_perm("view_report", report):
            raise PermissionDenied

        form = ReportProductListForm(request.user, request.GET)
