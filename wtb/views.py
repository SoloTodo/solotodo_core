...
from wtb.filters import (
    create_wtb_brand_filter,
    WtbEntityFilterSet,
    WtbBrandUpdateLogFilterSet,
    WtbEntityStaffFilterSet,
    WtbStoreFilterSet,
)
...
from wtb.serializers import (
    WtbBrandSerializer,
    WtbEntitySerializer,
    WtbBrandUpdateLogSerializer,
    WtbEntityStaffInfoSerializer,
)
...
class WtbEntityViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = ("name", "brand", "last_updated", "key", "category", "product")
    search_fields = (
    ...
    def pending(self, request):
        ...
        serializer = WtbEntitySerializer(page, many=True, context={"request": request})
    ...
    def available_entities(self, request, *args, **kwargs):
        ...
        if not wtb_entity.product:
            return Response(
                {
                    "detail": "Must be called on a WTB entity "
        ...
        es = (
            ...
            .select_related("active_registry", "product__instance_model", "best_coupon")
        )
    ...
    def toggle_visibility(self, request, *args, **kwargs):
        ...
        try:
            ...
            serialized_data = WtbEntitySerializer(
                wtb_entity, context={"request": self.request}
            ).data
    ...
    def available_alternatives(self, request, pk):
        ...
        if not wtb_entity.product_id:
            return Response({"error": "Please select an associated WTB Entity"})

        wtb_brand = wtb_entity.brand

        filterset = WtbStoreFilterSet(
            queryset=wtb_brand.stores.all(), data=request.query_params, request=request
        )
        ...
        similar_products = product.find_similar(
            stores=stores, initial_candidate_entities=entities
        )["similar"]
        ...
        similar_wtb_entities = [
            product_wtb_entity_dict[product] for product in similar_products
        ]

        serialized_data = WtbEntitySerializer(
            similar_wtb_entities, many=True, context={"request": self.request}
        ).data
        return Response(serialized_data)
