...
class ReportCurrentPricesForm(forms.Form):
    ...
    store_types = forms.ModelMultipleChoiceField(
        queryset=StoreType.objects.all(), required=False
    )
    ...
    extended = forms.BooleanField(required=False)
    ...
    def generate_report(self, es_product_search=None):
        ...
        if category:
            entities_filter &= Q(product__instance_model__model__category=category)
        ...
        if normal_price_usd_min:
            entities = entities.filter(normal_price_usd__gte=normal_price_usd_min)

        if normal_price_usd_max:
            entities = entities.filter(normal_price_usd__lte=normal_price_usd_max)
        ...
        self.generate_worksheet(
            workbook, category, currency, entities, es_dict, extended, display_sii_data
        )
    ...
    def generate_worksheet(
        workbook, category, currency, es, es_dict, extended, display_sii_data
    ):
        ...
        if category:
            specs_columns = CategoryColumn.objects.filter(
                field__category=category, purpose=settings.REPORTS_PURPOSE_ID
            )
        ...
        cell_plan_installments = {"Movistar": 18, "Entel": 18, "Claro": 12, "WOM": 18}
        ...
        if cell_plans_in_entities:
            ...
            cell_plan_entities = (
                Entity.objects.filter(product__in=cell_plans_in_entities.keys())
                .get_available()
                .values("product")
                .annotate(min_price=Min("active_registry__normal_price"))
            )

            cell_plan_prices_dict = {
                e["product"]: e["min_price"] for e in cell_plan_entities
            }

        headers.extend(["Tienda", "Seller"])

        if display_sii_data:
            headers.extend(["RUT Tienda", "Razón Social Tienda"])
        ...
        for e in es:
            ...
            if cell_plans_in_entities:
                ...
                if cell_plan:
                    cell_plan_price = cell_plan_prices_dict.get(cell_plan.id, "N/A")

                    worksheet.write(row, col, str(cell_plan))
                    worksheet.write(row, col + 1, str(cell_plan.specs["base_name"]))
                    ...
                    worksheet.write(row, col + 3, cell_plan.specs["lease_unicode"])
            ...
            if cell_monthly_payments_in_entities:
                if e.active_registry.cell_monthly_payment is not None:
                    worksheet.write(row, col, e.active_registry.cell_monthly_payment)
                    ...
                    if e.active_registry.cell_monthly_payment and e.cell_plan:
                        ...
                        installments = cell_plan_installments.get(plan_brand, "N/A")
            ...
            # Review count
            review_count = e.review_count if e.review_count is not None else "N/A"
            worksheet.write(row, col, review_count)
            col += 1

            # Review score
            review_avg_score = (
                e.review_avg_score if e.review_avg_score is not None else "N/A"
            )
            ...
            has_virtual_assistant = (
                e.has_virtual_assistant
                if e.has_virtual_assistant is not None
                else "N/A"
            )
